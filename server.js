...
//---------------------------test routes
app.get('/test/cities-count', async (req, res) => {
  try {
    const count = await City.count();
    return res.json({ count });
  } catch (err) {
...
app.get('/test/cities', async (req, res) => {
  try {
...
app.get('/test/cities/in', async (req, res) => {
  try {
    const cities = await City.findAll({ where: { country_code: 'IN' } });
    return res.json(cities);
  } catch (err) {
    return res.status(500).json({ error: 'Error fetching cities for IN' });
  }
});
app.get('/test/match/mum', async (req, res) => {
  try {
    const cities = await City.findAll({
      where: {
        country_code: 'IN',
    ...
    return res.status(500).json({ error: 'Match test failed' });
  }
});
////--------------------------------------------
